...
import { deleteCartItem, getProduct } from "@/src/app/api/fetchApi/admin";
...
export default function ProductCardForCart({ p, onQuantityChange }: Props) {
  ...
  const handleQuantityChange = async (newQty: number) => {
    ...
    await fetch("/api/cart", {
      method: "PUT",
      headers,
      body: JSON.stringify({ productId: p.productId, quantity: newQty }),
    });

    onQuantityChange?.(p.productId!, newQty);
  };
      ...
      <div className="flex flex-col justify-between flex-1 p-4 sm:p-6">
        <div>
          <div className="flex items-start justify-between">
            <h1 className="text-base sm:text-lg font-medium leading-snug">{product?.name}</h1>
            <div className="flex items-center gap-3 shrink-0 ml-4">
              <button aria-label="Favorite"><Heart className="w-5 h-5" /></button>
              <button aria-label="Remove" onClick={() => handleDeleteItem(String(p.productId))}><X className="w-5 h-5" /></button>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mt-4 sm:mt-6">
          ...
          {/* Ціна з урахуванням кількості */}
          <h3 className="text-base sm:text-lg font-medium">
            {((product?.price ?? 0) * quantity).toFixed(0)} UAH
          </h3>
        </div>
      </div>
    </div>
